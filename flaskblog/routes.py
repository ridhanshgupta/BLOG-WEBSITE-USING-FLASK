...
def account():
    ...
    profile_image = url_for('static', filename='profile_pics/' + current_user.image_file)
...
def new_post():
    ...
    if post.validate_on_submit():
        new_post = Post(title=post.title.data, content=post.content.data, author=current_user)
        db.session.add(new_post)
        db.session.commit()
        flash('Your post has been created!', 'success')
        return redirect(url_for('home'))
    return render_template('new_post.html', title='New Post' , form=post , legend='New Post')
...
def user_posts(username):
    ...
    return render_template('user_posts.html', posts=posts, user=user ,title=f"{user.username}'s Posts")